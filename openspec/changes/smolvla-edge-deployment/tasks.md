...
- [ ] 2.3 Fine-tune SmolVLA on the ALOHA dataset — **smoke run done locally** (1k steps, batch 4, loss 0.34→0.08, 2 GB VRAM, ~5 min on RTX 2000 Ada; checkpoint evals closed-loop and already grasps, reward 2/4 in 1/3 eps). Full 20k-step run pending a big GPU (Colab A100): `BATCH_SIZE=64 STEPS=20000 docker compose run --rm train`
